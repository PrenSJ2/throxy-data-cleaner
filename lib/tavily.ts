...
export async function searchCompanyWebsite(companyName: string) {
    const resp = await tavilyClient.search(`${companyName} official website`, { type: "text", searchDepth: "basic" });
...
export async function getCompanyStockTicker(companyName: string) {
    const resp = await tavilyClient.search(`${companyName} company stock ticker`, { type: "text", searchDepth: "basic" });
...
export async function getCompanyValue(companyName: string) {
    const resp = await tavilyClient.search(`${companyName} valuation`, { type: "text", searchDepth: "basic" });
    return await extractInfoFromTavilyResponse("valuation, (as a unit of billions)", resp.results?.[0]?.content);
}

export async function getCompanyCEO(companyName: string) {
    const resp = await tavilyClient.search(`${companyName} CEO`, { type: "text", searchDepth: "basic" });
...
export async function getCompanyEmployeeCount(companyName: string) {
    const resp = await tavilyClient.search(`${companyName} number of employees`, { type: "text", searchDepth: "basic" });
